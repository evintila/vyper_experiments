...
class FunctionAnalyzer(VyperNodeVisitorBase):
    ...
    def analyze(self):
        ...
        for arg in self.func.arguments:
            self.namespace[arg.name] = VarInfo(
                arg.typ, name=arg.name, location=location, modifiability=modifiability, decl_node=arg.ast_source
            )
    ...
    def visit_AnnAssign(self, node):
        ...
        self.namespace[name] = VarInfo(typ, name=name, location=DataLocation.MEMORY, decl_node=node)
    ...
    def visit_For(self, node):
        ...
        with self.namespace.enter_scope(), self.enter_for_loop(iter_var):
            ...
            self.namespace[target_name] = VarInfo(
                target_type, name=target_name, modifiability=Modifiability.RUNTIME_CONSTANT, decl_node=node.target
            )
